...
/**
 * Scrape Kworb data for a given artist.
 *
 * @param artistName - The artist's name used in the URL.
 * @param type - The type of data to scrape: "tracks" or "videos".
 * @returns The scraped data. For type "tracks", returns an object { stats, tracks }.
 *          For type "videos", returns an object { videos }.
 */
export const scrapeKworbData = async (
  ...
  try {
    ...
    if (type === 'tracks') {
      // Scrape tracks using similar logic as in the route
      ...
      // Scrape videos using similar logic as in the video route
