// AWS Lambda function for Kworb Spotify listeners scraper
const playwright = require('playwright-aws-lambda');
const { createClient } = require('@supabase/supabase-js');

// Interface for the scraped data
/**
 * @typedef {Object} SpotifyListenerData
 * @property {string} artist
 * @property {string} spotify_id
 * @property {number} listeners
 * @property {number} dailyTrend
  ...
  try {
    ...
    artists.forEach(artist => {
      if (artist.artist_platform_ids && Array.isArray(artist.artist_platform_ids)) {
        artist.artist_platform_ids.forEach((platformId) => {
    ...
    /** @type {Array<{artist_id: string, platform: string, metric_type: string, value: number}>} */
...
/**
 * Scrapes Kworb for Spotify listener data
 * @param {number} maxRetries - Maximum number of retry attempts
 * @returns {Promise<SpotifyListenerData[]>}
 */
async function scrapeKworbSpotifyListeners(maxRetries = 3) {
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      // Use playwright-aws-lambda for browser management
