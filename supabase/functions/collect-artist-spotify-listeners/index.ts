...
serve(async (req: Request) => {
  try {
    ...
    if (requestData && Array.isArray(requestData) && requestData.length > 0 && requestData[0].data) {
      ...
      console.log('No Bright Data results in request, will fetch artists from database');
    }
    ...
    console.error('Error in Spotify listeners collection:', error);
    return new Response(JSON.stringify({
      success: false,
      error: 'Failed to run Spotify listeners collection',
