...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) {
      return new Response(JSON.stringify({ error: 'Artist id is required' }), { status: 400 });
    }
    ...
    try {
      ...
      // Artist URLs
      const { error: urlError } = await supabase.from('artist_urls').delete().eq('artist_id', id);
      ...
      const { error: artistError } = await supabase.from('artists').delete().eq('id', id);
      ...
      return new Response(JSON.stringify({ message: 'Artist deleted successfully.', id }), {
        status: 200,
      });
    } catch (error) {
