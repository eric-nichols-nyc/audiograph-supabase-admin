...
const artistVideosSchema = z.object({ artist_id: z.string() });

const artistSlugSchema = z.object({ slug: z.string() });
...
export const getArtistVideos = actionClient
  ...
  .action(async ({ parsedInput }: { parsedInput: { artist_id: string } }): Promise<Video[]> => {
    ...
    if (connectionError)
      throw new Error(`Error fetching video connections: ${connectionError.message}`);
